...
def HomePage():
    ...
    # log in button
    button_about = Button(My_Window, text="Admin", font=("Arial", 12, 'bold'), bg="#E4E7E8",
                          activebackground="#E4E7E8",
                          foreground="#242424", activeforeground="blue", height=1, width=8, borderwidth=0, bd=0,
                          relief=FLAT,
...
def AboutPage():
    ...
    # log in button
    button_about = Button(My_Window, text="Admin", font=("Arial", 12, 'bold'), bg="#E4E7E8", activebackground="#E4E7E8",
...
def RegisterPage():
    ...
    validation = My_Window.register(on_validate_input)
...
def Easy():
    ...
    mediumbutton.place(x=548, y=120)
    ...
    def easybuttons(buttons):
        ...
        if buttons == 1:
            ...
            cursor = cnx.cursor()
            cursor.execute("SELECT Name, Status FROM memberdetails WHERE Status = 'Married'") # Replace "your_table" with the actual table name
            ...
            cursor = cnx.cursor()
            cursor.execute("SELECT MIDNumber, Name, PhoneNumber FROM memberdetails WHERE SEX ='M'")  # Replace "your_table" with the actual table name
            column_names = [column[0] for column in cursor.description]
            rows = cursor.fetchall()
...
def Moderate():
    ...
    def easybuttons(buttons):
        ...
        if buttons == 1:
            cursor = cnx.cursor()
            cursor.execute(
                "SELECT Sex, Name, TIMESTAMPDIFF(YEAR, Birthdate, CURDATE()) AS Age "
                "FROM memberdetails "
                "WHERE TIMESTAMPDIFF(YEAR, Birthdate, CURDATE()) >= 40 "
                "ORDER BY Sex, Name;")  # Replace "your_table" with the actual table name
            column_names = [column[0] for column in cursor.description]
            rows = cursor.fetchall()
            ...
            cursor.execute(
                    ...
                    "FROM workdetails as w, memberdetails as m, payment as p, employer e "
                    "WHERE m.MIDNumber = p.MIDNumber AND p.EmployeeNumber = w.EmployeeNumber AND w.EmployerID = e.EmployerID "
    ...
    def easybuttons(buttons):
        ...
        if buttons == 1:
            ...
            cursor.execute(
                ...
                "AND p.MIDNumber = m.MIDNumber AND year(m.birthdate) BETWEEN 1990 AND 2000 ORDER BY name;")  # Replace "your_table" with the actual table name
            column_names = [column[0] for column in cursor.description]
            rows = cursor.fetchall()
            ...
            cursor = cnx.cursor()
            cursor.execute(
                    "SELECT MSPayment, Name, month(birthdate) as Birthmonth, HeirsName, month(heirsbirthdate) as HeirBirthmonth "
            ...
            cursor = cnx.cursor()
            cursor.execute(
                    "SELECT employmentstatus, AVG(monthlyincome) as ave_monthlyincome "
                    "FROM workdetails "
                    "GROUP BY employmentstatus "
                    "HAVING ave_monthlyincome > 30000;")  # Replace "your_table" with the actual table name
            column_names = [column[0] for column in cursor.description]
            rows = cursor.fetchall()
            ...
            cursor = cnx.cursor()
            cursor.execute("SELECT Name, Occupation, Employer, EmployerAddress "
                           "FROM memberdetails as m, workdetails as w, payment p, employer e "
                           "WHERE m.MIDNumber = p.MIDNumber AND p.EmployeeNumber = w.EmployeeNumber AND w.EmployerID = e.EmployerID AND (e.EmployerAddress NOT LIKE '%MANILA%');")  # Replace "your_table" with the actual table name
            column_names = [column[0] for column in cursor.description]
            rows = cursor.fetchall()
